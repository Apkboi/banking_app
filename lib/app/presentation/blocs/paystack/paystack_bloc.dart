...
import '../../../data/models/server_error_model.dart';
...
class PaystackBloc extends Bloc<PaystackEvent, PaystackState> {
  ...
  Future<FutureOr<void>>_mapGetBankEventToState(GetBankEvent event,Emitter<PaystackState> state) async {
    try {
  ...
  Future<FutureOr<void>>_mapVerifyAccountEventToState(VerifyAccountEvent event,Emitter<PaystackState> state) async {
    try {
      emit(VerifyAccountLoadingState());

      final State? state = await paystackRepository.verifyBankAccount(event.accountNumber,event.bankCode);
      if (state is SuccessState) {
        // log(s);
